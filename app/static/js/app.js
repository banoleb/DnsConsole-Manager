...
createApp({
  ...
  computed: {
    filteredAgents() {
      ...
      if (this.selectedGroupFilter === "all") {
        ...
        return agents.filter((agent) => agent.group_id === Number(this.selectedGroupFilter));
      }
    },
    ...
    filteredAgentsWithIndex() {
      ...
      const indexMap = new Map(
        this.agents.map((agent, idx) => [agent.id, idx]).filter(([id]) => id !== undefined && id !== null),
      );
      ...
      return filtered.map((agent, fallbackIndex) => ({
        ...agent,
        originalIndex: agent.id != null ? (indexMap.get(agent.id) ?? fallbackIndex) : fallbackIndex,
      }));
    },
    filteredCommands() {
      ...
      return this.allCommands.filter((cmd) => cmd.toLowerCase().includes(search));
    },
    filteredHistory() {
  ...
  methods: {
    ...
    getStatusClass(status) {
      return status === "online" ? "status-online" : status === "offline" ? "status-offline" : "status-unknown";
    },
    ...
    handleAutocompleteKeydown(event, inputType, index = null) {
      const showDropdown =
        inputType === "broadcast" ? this.autocomplete.showBroadcast : this.autocomplete.showAgent[index];
