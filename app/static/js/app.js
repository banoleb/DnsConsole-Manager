...
createApp({
  ...
  computed: {
    ...
    totalPages() {
      return Math.ceil(
        this.historyPagination.total / this.historyPagination.perPage,
      );
    },
    hasPreviousPage() {
  ...
  methods: {
    ...
    getServersCount(agentId) {
      const agentServers = this.agentsServers.find(
        (as) => as.agent_id === agentId,
      );
    ...
    getTopClientsCount(agentId) {
      const agentTopClients = this.agentsTopClients.find(
        (atc) => atc.agent_id === agentId,
      );
    ...
    getTopQueriesCount(agentId) {
      const agentTopQueries = this.agentsTopQueries.find(
        (atq) => atq.agent_id === agentId,
      );
    ...
    getStatusIcon(status) {
      return status === "online" ? "✅" : status === "offline" ? "❌" : "⚠️";
    },
    async sendBroadcastCommand(command) {
      ...
      const targetLabel =
        this.broadcastTarget === "all"
          ? "all agents"
          : this.broadcastTarget === "none"
            ? "agents without group"
            : this.groups.find((g) => g.id === parseInt(this.broadcastTarget))
                ?.name || "selected group";
        ...
        data.results.forEach((result, index) => {
          ...
          if (result.success) {
            ...
            if (result.parsed_rules && Array.isArray(result.parsed_rules)) {
              ...
              html +=
                '<th style="padding: 6px; text-align: left; border: 1px solid #4a4a4a;">ID</th>';
              html +=
                '<th style="padding: 6px; text-align: left; border: 1px solid #4a4a4a;">Name</th>';
              html +=
                '<th style="padding: 6px; text-align: left; border: 1px solid #4a4a4a;">UUID</th>';
              html +=
                '<th style="padding: 6px; text-align: left; border: 1px solid #4a4a4a;">Sort</th>';
              html +=
                '<th style="padding: 6px; text-align: left; border: 1px solid #4a4a4a;">Matches</th>';
              html +=
                '<th style="padding: 6px; text-align: left; border: 1px solid #4a4a4a;">Rule</th>';
              html +=
                '<th style="padding: 6px; text-align: left; border: 1px solid #4a4a4a;">Action</th>';
              html += "</tr></thead><tbody>";
              ...
              result.parsed_servers &&
              Array.isArray(result.parsed_servers)
            ) {
              html += '<div style="margin-top: 10px;">';
              ...
              html +=
                '<th style="padding: 6px; text-align: left; border: 1px solid #4a4a4a;">Name</th>';
              html +=
                '<th style="padding: 6px; text-align: left; border: 1px solid #4a4a4a;">Address</th>';
              html +=
                '<th style="padding: 6px; text-align: left; border: 1px solid #4a4a4a;">State</th>';
              html +=
                '<th style="padding: 6px; text-align: left; border: 1px solid #4a4a4a;">QPS</th>';
              html +=
                '<th style="padding: 6px; text-align: left; border: 1px solid #4a4a4a;">Queries</th>';
              html +=
                '<th style="padding: 6px; text-align: left; border: 1px solid #4a4a4a;">Drops</th>';
              html +=
                '<th style="padding: 6px; text-align: left; border: 1px solid #4a4a4a;">Drate</th>';
              html +=
                '<th style="padding: 6px; text-align: left; border: 1px solid #4a4a4a;">Lat</th>';
              html +=
                '<th style="padding: 6px; text-align: left; border: 1px solid #4a4a4a;">TCP</th>';
              html +=
                '<th style="padding: 6px; text-align: left; border: 1px solid #4a4a4a;">Out</th>';
              html +=
                '<th style="padding: 6px; text-align: left; border: 1px solid #4a4a4a;">Pools</th>';
              html += "</tr></thead><tbody>";

              result.parsed_servers.forEach((server) => {
                const stateColor =
                  server.state === "up" ? "#28a745" : "#dc3545";
                html += '<tr style="border-bottom: 1px solid #3a3a3a;">';
      ...
      try {
        ...
        if (data.success) {
          ...
          if (data.parsed_rules && Array.isArray(data.parsed_rules)) {
            ...
            output +=
              '<th style="padding: 8px; text-align: left; border: 1px solid #4a4a4a;">ID</th>';
            output +=
              '<th style="padding: 8px; text-align: left; border: 1px solid #4a4a4a;">Name</th>';
            output +=
              '<th style="padding: 8px; text-align: left; border: 1px solid #4a4a4a;">UUID</th>';
            output +=
              '<th style="padding: 8px; text-align: left; border: 1px solid #4a4a4a;">Sort</th>';
            output +=
              '<th style="padding: 8px; text-align: left; border: 1px solid #4a4a4a;">Matches</th>';
            output +=
              '<th style="padding: 8px; text-align: left; border: 1px solid #4a4a4a;">Rule</th>';
            output +=
              '<th style="padding: 8px; text-align: left; border: 1px solid #4a4a4a;">Action</th>';
            ...
            output +=
              '<br><details style="margin-top: 10px;"><summary style="cursor: pointer; color: #6c757d;">Show raw output</summary>';
            ...
            data.parsed_servers &&
            Array.isArray(data.parsed_servers)
          ) {
            ...
            output +=
              '<th style="padding: 8px; text-align: left; border: 1px solid #4a4a4a;">Name</th>';
            output +=
              '<th style="padding: 8px; text-align: left; border: 1px solid #4a4a4a;">Address</th>';
            output +=
              '<th style="padding: 8px; text-align: left; border: 1px solid #4a4a4a;">State</th>';
            output +=
              '<th style="padding: 8px; text-align: left; border: 1px solid #4a4a4a;">QPS</th>';
            output +=
              '<th style="padding: 8px; text-align: left; border: 1px solid #4a4a4a;">Queries</th>';
            output +=
              '<th style="padding: 8px; text-align: left; border: 1px solid #4a4a4a;">Drops</th>';
            output +=
              '<th style="padding: 8px; text-align: left; border: 1px solid #4a4a4a;">Drate</th>';
            output +=
              '<th style="padding: 8px; text-align: left; border: 1px solid #4a4a4a;">Lat</th>';
            output +=
              '<th style="padding: 8px; text-align: left; border: 1px solid #4a4a4a;">TCP</th>';
            output +=
              '<th style="padding: 8px; text-align: left; border: 1px solid #4a4a4a;">Out</th>';
            output +=
              '<th style="padding: 8px; text-align: left; border: 1px solid #4a4a4a;">Pools</th>';
            output += "</tr></thead><tbody>";
        ...
        this.agentOutputs[index] =
          `<span style="color: #dc3545;">✗ Connection Error</span>\n` +
          "\nCommand: " +
          command +
          `\n${this.escapeHtml(error.message)}` +
          "\nCommand: " +
    ...
    handleBroadcastEnter() {
      ...
      if (
        !this.autocomplete.showBroadcast ||
    ...
    handleAgentEnter(agent, index) {
      ...
      if (
        !this.autocomplete.showAgent[index] ||
        this.autocomplete.suggestions.length === 0
      ) {
    ...
    async loadHistory() {
      try {
        const offset =
          ...
          this.historyPagination.perPage;
    ...
    async deleteAgent(agent) {
      if (
        !confirm(`Are you sure you want to delete agent "${agent.agent_name}"?`)
      ) {
        return;
      }
    ...
    async fetchAutocompleteSuggestions(query) {
      ...
      try {
        const response = await fetch(
          `/api/history/autocomplete?q=${encodeURIComponent(query)}&limit=10`,
        );
        const data = await response.json();
      ...
      switch (event.key) {
        ...
        case "ArrowRight":
          if (this.autocomplete.selectedIndex >= 0) {
            event.preventDefault();
            const suggestion =
              this.autocomplete.suggestions[this.autocomplete.selectedIndex];
